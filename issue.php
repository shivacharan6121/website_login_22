                    ...
                    <tr>
                        ...
                        <td>
                            <div class="action-buttons">
                                <button class="print-but" onclick="showPrintDocument(
                                ...
                                <button class="edit-btn" onclick="showEditDialog(
                                    '<?php echo htmlspecialchars($row['Nomenclature'], ENT_QUOTES); ?>',
                                    '<?php echo htmlspecialchars($row['make'], ENT_QUOTES); ?>',
                                    '<?php echo htmlspecialchars($row['person_name'], ENT_QUOTES); ?>',
                                    '<?php echo htmlspecialchars($row['designation'] ?? '', ENT_QUOTES); ?>',
                                    ...
                                    '<?php echo htmlspecialchars($row['issuedqty'], ENT_QUOTES); ?>',
                                    '<?php echo htmlspecialchars($row['timestamp'], ENT_QUOTES); ?>'
                                )">Edit</button>
    ...
    <div id="editModal" class="modal">
        <div class="modal-content" style="max-width: 600px;">
            <div class="modal-header">
                ...
                <h2>Edit Issue Record</h2>
            </div>
            <div class="modal-body">
                <form id="editForm">
                    <input type="hidden" id="original_nomenclature" name="original_nomenclature">
                    <input type="hidden" id="original_timestamp" name="original_timestamp">
                    
                    <div class="form-group">
                        <label>Part No:</label>
                        <input type="text" id="edit_nomenclature" class="form-control" readonly>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="edit_person_name" name="person_name" class="form-control" required>
                    </div>
                    ...
                    <div class="form-group">
                        <label for="edit_issuedqty">Issued Quantity:</label>
                        <input type="number" id="edit_issuedqty" name="issuedqty" min="1" class="form-control" required onchange="validateQuantity(this)">
                    </div>
                </form>
            </div>
            <div class="modal-buttons">
                <button type="button" class="cancel-btn" onclick="closeEditModal()">Cancel</button>
                <button type="button" class="apply-btn" onclick="submitEditForm()">Apply Changes</button>
            </div>
        </div>
    </div>
    <script>
        ...
        function confirmDelete(nomenclature, timestamp) {
            if (confirm('Are you sure you want to delete this record? This will add the issued quantity back to available stock.')) {
                deleteRecord(nomenclature, timestamp);
            }
        }

        function deleteRecord(nomenclature, timestamp) {
            fetch('delete_issue.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                },
                body: 'nomenclature=' + encodeURIComponent(nomenclature) + '&timestamp=' + encodeURIComponent(timestamp)
            })
            .then(response => response.json())
            .then(data => {
                if (data.success) {
        ...
        function showEditDialog(nomenclature, make, personName, designation, purpose, issuedQty, timestamp) {
            document.getElementById('original_nomenclature').value = nomenclature;
            document.getElementById('original_timestamp').value = timestamp;
            document.getElementById('edit_nomenclature').value = nomenclature;
            ...
            document.getElementById('edit_issuedqty').value = issuedQty;
            ...
            .then(data => {
                if (data.success) {
                    ...
                    if (data.error.includes('Quantity')) {
                        document.getElementById('edit_issuedqty').focus();
                    }
                }
            })
            .catch(error => {
        ...
        // Also update the window.onclick to handle the edit modal
        window.onclick = function(event) {
